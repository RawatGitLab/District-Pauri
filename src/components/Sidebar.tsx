...
import { Layers, Globe, Sliders, CheckSquare, Square, Check, RotateCcw, Database, ChevronDown, ChevronRight, Minimize2, Maximize2, Ruler, Trash2, Undo, GraduationCap, BookOpen, Heart, Shield, Waves, Compass, Trees, Mountain, Building2, Droplet } from "lucide-react";
...
const isRiverLayer = (name: string) => {
...
const isEducationLayer = (name: string) => {
  const lower = name.toLowerCase();
  return (
...
const isHealthLayer = (name: string) => {
  const lower = name.toLowerCase();
  return (
...
const isPoliceLayer = (name: string) => {
  const lower = name.toLowerCase();
  return lower.includes("police") || lower.includes("fire");
};

const isUrbanDevelopmentLayer = (name: string) => {
  const lower = name.toLowerCase();
  return (
    lower.includes("cantonment") ||
    lower.includes("cnatonment") ||
...
const isIrrigationLayer = (name: string) => {
  const lower = name.toLowerCase();
  return (
    lower.includes("irrigation") ||
    lower.includes("lift_scheme") ||
    lower.includes("tube_well") ||
    lower.includes("tubewell")
  );
};

const isAdministrativeLayer = (name: string) => {
  const lower = name.toLowerCase();
  
  const isPolice = isPoliceLayer(name);
  const isHealth = isHealthLayer(name);
  const isRiver = isRiverLayer(name);
  const isEducation = isEducationLayer(name);
  const isUrban = isUrbanDevelopmentLayer(name);
  const isIrrigation = isIrrigationLayer(name);

  if (isPolice || isHealth || isEducation || isRiver || isUrban || isIrrigation) return false;
...
const isLandUseLayer = (name: string) => {
  const lower = name.toLowerCase();
  if (isRiverLayer(name)) return false;
  if (LAND_USE_NAMES.has(lower)) return true;
  return lower.includes("landuse") || lower.includes("land-use");
};
...
export default function Sidebar({
  ...
  const [isLandUseCollapsed, setIsLandUseCollapsed] = useState<boolean>(true);
  ...
  const administrativeLayers = useMemo(() => {
    return layers.filter(layer => {
      return isAdministrativeLayer(layer.name);
    });
  }, [layers]);

  const urbanDevelopmentLayers = useMemo(() => {
    return layers.filter(layer => {
      return isUrbanDevelopmentLayer(layer.name);
    });
  }, [layers]);

  const irrigationLayers = useMemo(() => {
    return layers.filter(layer => {
      return isIrrigationLayer(layer.name);
    });
  }, [layers]);

  const educationLayers = useMemo(() => {
    return layers.filter(layer => {
      return isEducationLayer(layer.name);
    });
  }, [layers]);

  const healthLayers = useMemo(() => {
    return layers.filter(layer => {
      return isHealthLayer(layer.name);
    });
  }, [layers]);

  const policeLayers = useMemo(() => {
    return layers.filter(layer => {
      return isPoliceLayer(layer.name);
    });
  }, [layers]);

  const riverLayers = useMemo(() => {
    return layers.filter(layer => {
      return isRiverLayer(layer.name);
    });
  }, [layers]);

  const landUseLayers = useMemo(() => {
    return layers.filter(layer => {
      return isLandUseLayer(layer.name);
    });
  }, [layers]);

  const otherLayers = useMemo(() => {
    return layers.filter(layer => {
      const isPolice = isPoliceLayer(layer.name);
      const isHealth = isHealthLayer(layer.name);
      const isRiver = isRiverLayer(layer.name);
      const isEducation = isEducationLayer(layer.name);
      const isAdministrative = isAdministrativeLayer(layer.name);
      const isLandUse = isLandUseLayer(layer.name);
      const isUrban = isUrbanDevelopmentLayer(layer.name);
      const isIrrigation = isIrrigationLayer(layer.name);

      if (isPolice || isHealth || isRiver || isEducation || isAdministrative || isLandUse || isUrban || isIrrigation) {
        return false;
      }
    ...
    <aside className="w-80 border-r border-slate-200 bg-slate-50 flex flex-col h-full shrink-0 shadow-sm font-sans">
        ...
        <div className="space-y-3">
          ...
          {!isLayersCollapsed && (
            <div className="space-y-4">
              ...
              {/* Collapsible Urban Development Layer Section */}
              <div className="border border-slate-200 rounded-lg overflow-hidden bg-slate-50/30 shadow-sm">
                <button
              ...
              {/* Collapsible Irrigation Layer Section */}
              <div className="border border-slate-200 rounded-lg overflow-hidden bg-slate-50/30 shadow-sm">
                <button
              ...
              {/* Collapsible LAND-USE Layer Section */}
              <div className="border border-slate-200 rounded-lg overflow-hidden bg-slate-50/30 shadow-sm">
                <button
                  onClick={() => setIsLandUseCollapsed(!isLandUseCollapsed)}
                  className="w-full flex items-center justify-between p-2.5 bg-slate-100/80 hover:bg-slate-200/60 transition-colors text-left font-sans focus:outline-none border-b border-slate-200/60 cursor-pointer"
                >
                  <span className="text-xs font-bold text-slate-700 flex items-center gap-1.5 uppercase tracking-wider">
                    <Trees className="w-3.5 h-3.5 text-emerald-600" />
                    LAND-USE Layer ({landUseLayers.length})
                  </span>
                  {isLandUseCollapsed ? (
                    <ChevronRight className="w-3.5 h-3.5 text-slate-500" />
                  ) : (
                ...
                {!isLandUseCollapsed && (
                  <div className="bg-white divide-y divide-slate-100 max-h-96 overflow-y-auto">
                    {landUseLayers.length === 0 ? (
                      <div className="p-4 text-center text-xs text-slate-400 font-medium">
                        No land-use layers loaded.
                      </div>
                    ) : (
                      landUseLayers.map((layer) => renderLayerItem(layer))
                    )}
                  </div>
                )}
              </div>
      ...
      <div className="p-3 border-t border-slate-200 bg-white">
        <div className="bg-slate-50 border border-slate-100 rounded-md p-2 flex flex-col gap-1 text-[11px] text-slate-500 font-medium">
          ...
          <div className="flex justify-between">
            <span>District (Pauri):</span>
            <span className="font-mono text-slate-700 font-bold">061</span>
          </div>
        </div>
      </div>
    </aside>
