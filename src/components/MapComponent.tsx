...
export default function MapComponent({
  ...
  useEffect(() => {
    ...
    // Center on Pauri, Uttarakhand, India (Pauri: 29.8476, 78.7185)
    const map = L.map(mapContainerRef.current, {
      center: [29.8476, 78.7185],
      zoom: 10,
  ...
  // Map Controls: Pan to Pauri bounds safely
  const handleZoomToDistrict = () => {
    ...
    // Fallback static center of Pauri
    map.setView([29.8476, 78.7185], 10, { animate: true });
  };
              ...
              return (
                ...
                >
                  ...
                  <span className="text-[9px] uppercase tracking-wider bg-slate-100 text-slate-400 font-bold px-1.5 py-0.5 rounded font-mono shrink-0">
                    {layerName.replace("USN-", "").replace("Pauri-", "").replace("-Boundary", "")}
                  </span>
                </button>
              );
            })}
